/*
	...
	说明：
	用法（二选一）：
	deno run --allow-read --allow-write --allow-net P:\2022\20220613a\3code\9_tools\deno\imageHelper\createStoriesImages.ts
	cls && deno run --allow-read --allow-write --allow-net P:\2022\20220613a\3code\9_tools\deno\imageHelper\createStoriesImages.ts
	
	查看注释
	P: && CD P:\2022\20220613a\3code\9_tools\deno\imageHelper\ && deno doc createStoriesImages.ts
*/
...
const GOAL_PATH: string = 'P:/ecs_person/websites/sonya.cc/edu_git/src/images/stories/';
const IS_PNG = true;
...
for(let page = 1; page <= 50; ++page) {
	const name = `${page}`;
	const link = `http://edu.sonya.cc/?go=stories&page=${page}`;
	create(name, link);
