/*
	...
	说明：
	用法（二选一）：
	deno run --allow-read --allow-write --allow-net P:\2022\20220613a\3code\9_tools\deno\imageHelper\createBricksImages.ts
	cls && deno run --allow-read --allow-write --allow-net P:\2022\20220613a\3code\9_tools\deno\imageHelper\createBricksImages.ts
	
	查看注释
	P: && CD P:\2022\20220613a\3code\9_tools\deno\imageHelper\ && deno doc createBricksImages.ts
*/
...
const GOAL_PATH: string = 'P:/ecs_person/websites/sonya.cc/edu_git/src/images/bricks/';
const IS_PNG = true;
...
for(let kind = 0; kind < 10; ++kind) {
	for(let page = 1; page <= 10; ++page) {
		const name = `${kind}_${page}`;
		const link = `http://edu.sonya.cc/?go=bricks&kind=${kind}&page=${page}`;
		create(name, link);
